class WebGLUtils {
  ...
  getProgram = (gl, vertexShaderSource, fragmentShaderSource) => {
    const vs = this.getShader(gl, vertexShaderSource, gl.VERTEX_SHADER);
    const fs = this.getShader(gl, fragmentShaderSource, gl.FRAGMENT_SHADER);
  ...
  createAndBindFramebuffer = (gl, image) => {
    const texture = this.createAndBindTexture(gl, image);
  ...
  getCircleCoordinates = (centerX, centerY, radiusX, numOfPoints, isLine) => {
    ...
    for (let i = 0; i < numOfPoints; i++) {
      //2*Math.PI*r
      const circumference = 2 * Math.PI * (i / numOfPoints);
  ...
  getAspectRatio = (gl, img) => {
    ...
    const imageAR = cols / rows;
    const canvasAR = gl.canvas.width / gl.canvas.height;
    let startX, startY, renderableW, renderableH;
    if (imageAR < canvasAR) {
