...
float cross2(vec2 loc, vec2 size, vec2 coord){
...
void main() {
  ...
  float rad = 0.5;
  vec2 loc = vec2(0.0) + vec2(sin(time)*rad, cos(time)*rad);
  
  coord += loc;
  vec3 colorData = vec3(cross2(vec2(0.0), vec2(0.2, 0.02), coord));

  // vec3 colorData = vec3(cross2(loc, vec2(0.2, 0.02), coord));
